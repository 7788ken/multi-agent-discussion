...
import { normalizeRootPath, registerRoot } from './discussion-roots.js'
...
class Discussion {
  ...
  constructor(baseDir = DEFAULT_BASE_DIR) {
    this.baseDir = normalizeRootPath(baseDir) || path.normalize(path.resolve(baseDir))
    this.sleepArray = new Int32Array(new SharedArrayBuffer(4))
    this.ensureDir()
    try {
      registerRoot(this.baseDir)
    } catch {
      // Root registration is best effort; discussion I/O should continue.
    }
  }
  ...
  create(topic, participants = ['claude', 'codex'], context = {}) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        // Use exclusive create to avoid overwriting an existing discussion on ID collision.
    ...
    throw new Error('Failed to create discussion: could not allocate unique discussion id')
  }
    ...
    for (const msg of messages) {
      ...
      switch (msg.type) {
        ...
        case MESSAGE_TYPES.MODE:
          content += `### ⚙️ Mode Update - ${time}\n`
          content += `${msg.key}: ${msg.enabled ? 'on' : 'off'}\n\n`
          break
        default:
          break
      }
  ...
  getStatus(discussionId) {
    ...
    const lastModified = lastMessage?.ts ? new Date(lastMessage.ts).getTime() : Date.now()
    ...
    for (const msg of messages) {
      ...
      switch (msg.type) {
        ...
        case MESSAGE_TYPES.FOLLOWUP:
          ...
          break
        case MESSAGE_TYPES.MODE:
          output += `${prefix}\n  Mode: ${msg.key} => ${msg.enabled ? 'on' : 'off'}\n\n`
          break
