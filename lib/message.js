 ...
 * @property {string} type - Message type: 'start', 'response', 'followup', 'mode', 'end', 'error'
 ...
 * @property {string} [key] - Mode key (for 'mode' type)
 * @property {boolean} [enabled] - Mode switch value (for 'mode' type)
...
const MESSAGE_TYPES = {
  ...
  MODE: 'mode',         // Discussion mode toggle
...
/**
 * Create a mode toggle message
 * @param {number} seq
 * @param {string} key
 * @param {boolean} enabled
 * @returns {Message}
 */
function createModeMessage(seq, key, enabled) {
  const normalizedKey = String(key || '').trim().toLowerCase()
  const normalizedEnabled = Boolean(enabled)
  return createMessage({
    seq,
    from: 'user',
    type: MESSAGE_TYPES.MODE,
    key: normalizedKey,
    enabled: normalizedEnabled,
    content: `mode ${normalizedKey} ${normalizedEnabled ? 'on' : 'off'}`
  })
}
