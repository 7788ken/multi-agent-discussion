...
class AgentBase {
  ...
  async handleRetriableError(discussionId, trigger, allMessages, errorMessage, round, reason = 'failed') {
    ...
    if (!this.pendingRetries.has(discussionId)) {
      this.pendingRetries.set(discussionId, { attempts: 0, maxAttempts: 10 })
    }
  ...
  formatContextForLLM(discussionId, messages) {
    ...
    if (coDevEnabled) {
      ...
      context += `- Propose concrete next implementation steps to move the project forward.\n`
      context += `- Point out bugs/risks with file paths when possible.\n\n`
      context += `Co-Development Execution Policy:\n`
      context += `- You are allowed to directly edit project files for small, reversible improvements.\n`
      ...
      const snapshot = this.getProjectSnapshot(status.context?.workingDir || process.cwd())
      context += `${snapshot}\n\n`
    } else {
      context += '\n'
    }
