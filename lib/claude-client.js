...
async function callClaude(prompt, options = {}) {
  const {
    model = 'sonnet',
    timeout = 60000,  // 60 秒（之前是 180 秒）
    workingDir = '',  // 空字符串表示不指定工作目录
    discussionId = null
  } = options

  // Build args with session resume if available
  const args = [
    '-p',
    '--model', model,
  ...
  if (cachedSession) {
    args.push('--resume', cachedSession)
  }
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code, signal) => {
      ...
      if (code === 0 && stdout.trim()) {
        // Extract session ID from output if present (Claude CLI may output it)
